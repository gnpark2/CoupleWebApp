...
export class WorldScene extends Phaser.Scene {
  ...
  // Partner position is driven externally (via WebSocket); we lerp toward it
  ...
  init(config: WorldSceneConfig) {
    this.cfg = config
  }

  preload() {
    // No external assets — everything is drawn with Phaser graphics + text (emoji)
  }
  ...
  create() {
    ...
    border.lineStyle(2, 0xd8d4cc, 1)
    border.strokeRect(0, 0, WORLD_WIDTH, WORLD_HEIGHT)

    // ── My character ────────────────────────────────────
    this.myChar = this.createCharacter(120, 480, this.cfg.me, true)
  ...
  update(_time: number, delta: number) {
    ...
    // Smoothly move partner toward their target position
  ...
  /** Called externally when partner's position updates via WebSocket */
  ...
  private drawRoom(room: RoomDef) {
    ...
    g.fillRoundedRect(room.x, room.y, room.width, room.height, 12)
    g.strokeRoundedRect(room.x, room.y, room.width, room.height, 12)

    // Icon
    this.add.text(room.x + room.width / 2, room.y + room.height / 2 - 10, room.icon, {
      fontSize: '28px',
    }).setOrigin(0.5)

    // Label
    this.add.text(room.x + room.width / 2, room.y + room.height / 2 + 22, room.name, {
      fontSize: '12px',
      color: '#444441',
  ...
  private createCharacter(x: number, y: number, char: PlayerCharacter, isMe: boolean) {
    ...
    const label = this.add.text(0, 28, char.name, {
      fontSize: '11px',
      color: '#ffffff',
  ...
  private checkRoomProximity(x: number, y: number) {
    ...
    for (const room of ROOMS) {
      ...
      const radius = Math.max(room.width, room.height) / 2 + PROXIMITY_RADIUS - 40
      if (dist < radius) {
        found = room.id
        break
      }
